...
<div class="subheader-image-bg">
  <div class="bg-image-wrapper">
    ...
    <div class="bg-image" style="background: linear-gradient(0deg, rgba(25,24,24,1) 0%, rgba(25,24,24,0.8) 30%, rgba(25,24,24,0) 80%), url({{$user->avatar_square}});"></div>
  </div>
...
@section('user-content')
  ...
  <div class="flex-center-space m-b-20">
    <div class="flex-center">
      {{-- User Avatar --}}
      <span class="hidden-xs-down avatar profile-avatar m-r-20">
        <img src="{{$user->avatar_square}}" alt="{{$user->name}}'s Avatar">
      </span>
      <div>
        {{-- User Name & Location --}}
        ...
        @if($user->location)
          <img src="{{ trans('general.url') . 'img/flags/' .   $user->location->country_abbreviation . '.svg' }}" height="16"/> {{$user->location->country_abbreviation}}, {{$user->location->place}} <span class="postal-code">{{$user->location->postal_code}}</span>
        @endif
        </span>
