            ...
            <div class="input-group">
              <label class="input-group-btn">
                <span class="btn {{ $errors->has('avatar') ? 'bg-danger' : 'bg-success' }}">
                  <i class="fa fa-file-image-o" aria-hidden="true"></i> {{ trans('users.dash.settings.browse') }}&hellip; <input type="file" name="avatar" style="display: none;" multiple>
                </span>
              </label>
      ...
      <div class="input-wrapper">
        ...
        <div class="flex-center">
          ...
          <div class="current-location m-r-10">
            <img src="{{ trans('general.url') . 'img/flags/' .   $location->country_abbreviation . '.svg' }}" height="14"/> {{$location->country_abbreviation}}, {{$location->place}} <span class="postal-code">{{$location->postal_code}}</span>
          </div>
