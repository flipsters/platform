...
<div class="flex-center wrap m-b-20">
  ...
  <a href="{{ url('dash/notifications') }}" class="quick-action">
    {{-- Icon with count --}}
    <div class="quick-icon">
      <i class="fa fa-bell"></i>@if(count(Auth::user()->unreadNotifications)>0)<span class="badge badge-danger badge-sm up">{{count(Auth::user()->unreadNotifications)}}</span> @endif
    </div>
    <div class="quick-text">
...
@if($active_listings > 0)
  <div class="panels-title border-bottom flex-center-space">
    ...
    {{-- Show all link --}}
    <div>
      <a href="{{ url('dash/listings') }}" class="btn btn-dark f-w-500"><i class="fa fa-angle-right" aria-hidden="true"></i> <span class="hidden-xs-down">{{ trans('users.dash.show_all') }}</span></a>
    </div>
  </div>
    ...
    <section class="panel @if(!is_null($listing->deleted_at)) grayscale @endif">
      ...
      <div class="listing-body">
        ...
        <div class="listing {{ !is_null($offer->thread) && $offer->thread->isUnread(auth()->user()->id) ? 'notify' : '' }}" style="position: relative;">
            ...
            <div class="listing-detail">
              <div class="listing-detail-fix flex-center">
                ...
                <a href="{{$offer->user->url}}" class="offer-user flex-center">
                  {{-- Avatar --}}
                  <div class="avatar m-r-10">
                    <img src="{{ $offer->user->avatar_square_tiny }}" alt="{{ $offer->user->name }}'s Avatar'">
                  </div>
                  <div>
                    {{-- Username --}}
          ...
          @if($offer->status == 1 && $offer->listing->payment && !$offer->payment)
            <a href="{{ $offer->url }}">
        ...
        <div class="listing-no-offers">
          <i class="fa fa-frown-o" aria-hidden="true"></i> {{ trans('users.dash.listings.no_offers') }}
        </div>
        @endforelse
      ...
      <div class="panel-footer @if(!is_null($listing->deleted_at))padding @endif">
        ...
        <div class="listing-footer-time @if(!is_null($listing->deleted_at))deleted @endif">
          ...
          <i class="fa fa-bar-chart" aria-hidden="true"></i> {{$listing->clicks}} {{ trans('users.dash.listings.clicks') }}
        </div>
        {{-- Footer Buttons --}}
        <div>
        @if(is_null($listing->deleted_at))
          ...
          <a href="javascript:void(0)" data-toggle="modal" data-target="#modal_delete_{{$listing->id}}" class="button additional">
            ...
            <i class="fa fa-caret-square-o-right" aria-hidden="true"></i><span class="hidden-sm-down"> {{ trans('general.details') }}</span>
          </a>
        @endif
        </div>
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            ...
            {{-- Close button --}}
            <a href="#" data-dismiss="modal" data-bjax class="btn btn-lg btn-dark">{{ trans('general.cancel') }}</a>
...
@if(count($user->offers->where('status',0)->where('declined',0)) + count($user->offers->where('status',1)->where('declined',0)) > 0)
  <div class="panels-title border-bottom flex-center-space">
    ...
    {{-- Show all link --}}
    <div>
      <a href="{{ url('dash/offers') }}" class="btn btn-dark f-w-500"><i class="fa fa-angle-right" aria-hidden="true"></i> <span class="hidden-xs-down">{{ trans('users.dash.show_all') }}</span></a>
    </div>
  </div>
            ...
            <div class="listing-detail">
              <div class="listing-detail-fix flex-center">
                ...
                <a href="{{$offer->listing->user->url}}" class="offer-user flex-center">
                  {{-- Avatar --}}
                  <div class="avatar m-r-10">
                    <img src="{{ $offer->listing->user->avatar_square_tiny }}" alt="{{ $offer->listing->user->name }}'s Avatar'">
                  </div>
                  <div>
                    {{-- Username --}}
                    ...
                    {{-- User location --}}
                    <img src="{{ trans('general.url') . 'img/flags/' .   $offer->listing->user->location->country_abbreviation . '.svg' }}" height="14"/> {{$offer->listing->user->location->country_abbreviation}}, {{$offer->listing->user->location->place}} <span class="postal-code">{{$offer->listing->user->location->postal_code}}</span>
                    @endif
                  </div>
                </a>
                {{-- End offer user --}}
...
<div class="panels-title border-bottom flex-center-space">
  {{-- Title with active listings count --}}
  <div>
    <i class="fa fa-bar-chart" aria-hidden="true"></i> {{ trans('users.dash.stats.stats') }}
  </div>
</div>
...
<div class="flex-center-space-wrap">
  ...
  <div class="stats-counter no-flex-shrink">
    <span class="counter">{{ $user->listings->sum('clicks') }} <i class="fa fa-dot-circle-o" aria-hidden="true"></i></span>
