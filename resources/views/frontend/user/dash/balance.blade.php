...
@section('content')
  ...
  @forelse($transactions as $transaction)
    ...
    <div class="listing m-t-10 hvr-grow-shadow2">
      ...
      <div class="transaction-details {{ $transaction->type == 'sale' ? 'text-success' : 'text-danger' }} bg-dark">
        {{ money(abs(filter_var(number_format($transaction->total,2), FILTER_SANITIZE_NUMBER_INT)), $transaction->currency)->format(true) }}
      </div>
      ...
      {{-- Details Button --}}
      @if($transaction->type == 'fee' || $transaction->type == 'sale')
      ...
      <div>
        ...
        <a href="{{$transaction->payer->url}}" class="user-link">
          <span class="avatar avatar-xs">
            <img src="{{ $transaction->payer->avatar_square_tiny }}" alt="{{$transaction->payer->name}}'s Avatar">
          </span>
