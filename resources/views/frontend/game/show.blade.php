    ...
    <ul class="subheader-tabs" role="tablist">
      ...
      <li class="nav-item">
        <a data-toggle="tab" href="{{ url('/games/' . $game->id . '/media') }}" data-target="#media" role="tab" class="subheader-link">
          <i class="fa fa-picture-o" aria-hidden="true"></i><span class="{{ config('settings.comment_game') ? 'hidden-sm-down' : 'hidden-xs-down'}}"> {{ trans('games.overview.subheader.media') }}</span>
        </a>
      </li>
      @endif
      {{-- Comments tab --}}
...
app_id={{config('settings.facebook_client_id')}}&display=popup&href={{URL::current()}}&redirect_uri={{ url('self.close.html')}}" onclick="window.open(this.href, 'facebookwindow','left=20,top=20,width=600,height=400,toolbar=0,resizable=1'); return false;" class="btn btn-icon btn-round btn-lg social-facebook m-r-5">
    ...
    {{-- Twitter share --}}
    <a href="https://twitter.com/intent/tweet?text={{trans('general.share.twitter_game', ['game_name' => $game->name, 'platform' => $game->platform->name, 'page_name' => config('settings.page_name')])}} &#8921; {{URL::current()}}" onclick="window.open(this.href, 'twitterwindow','left=20,top=20,width=600,height=300,toolbar=0,resizable=1'); return false;" class="btn btn-icon btn-round btn-lg social-twitter m-r-5">
...
<div class="tab-content subheader-margin">
    ...
    <div class="listing-user-details flex-center-space">
      ...
      <div class="no-flex-shrink">
        ...
        @if($listing->user->location)
          {{$listing->user->location->place}} <img src="{{ trans('general.url') . 'img/flags/' .   $listing->user->location->country_abbreviation . '.svg' }}" height="14"/>
        @endif
        </span>
      </div>
    </div>
  ...
  <div class="form-inline m-t-50">
    ...
    <a href="{{ url('games/' . $game->id . '/refresh/metacritic') }}" class="btn btn-dark m-r-5 m-t-10" id="refresh-metacritic"><i class="fa fa-refresh"></i> Refresh Metacritic</a>
    @endif
    {{-- Change giantbomb id --}}
