...
<div class="listing-form {{ isset($game) ? '' : 'hidden' }}">
  ...
  <section class="panel">
    ...
    {{-- Panel Title (Details) --}}
    <div class="panel-heading">
      <h3 class="panel-title">{{ trans('listings.form.details_title') }}</h3>
    </div>
    ...
    <div class="panel-body">
      <div class="row no-space">
        <div class="form-group row">
          ...
          <div class="col-sm-6 {{ isset($game) ? $game->platform->digitals->count() > 0 ? '' : 'hidden' : '' }}" id="digital-input">
            <div class="checkbox-custom checkbox-default checkbox-lg">
              <input type="checkbox" id="digital" name="digital" autocomplete="off" data-platform="{{ isset($game) ? $game->platform->acronym : '' }}" {{ (isset($listing) && $listing->digital) ? 'checked' : '' }} />
              <label for="digital">
                {{ trans('listings.form.details.digital') }}
              </label>
            </div>
            {{-- Digital ditributor selection --}}
            <div class="{{ (isset($listing) && $listing->digital) ? '' : 'hidden' }}" id="digital-select">
        ...
        <div class="form-group row">
          ...
          <div class="col-sm-6">
            <div class="checkbox-custom checkbox-default checkbox-lg">
              <input type="checkbox" id="pickup" name="pickup" data-validation="delivery_pickup_check" data-validation-event="click" data-validation-error-msg-container="#delivery-pickup-error-dialog" {{ ( !isset($listing) || (isset($listing) &&  $listing->pickup == 1) ? 'checked' : '') }} />
              <label for="pickup">
          ...
          <div class="col-sm-6">
            <div class="checkbox-custom checkbox-default checkbox-lg" >
              <input type="checkbox" id="delivery" name="delivery" data-validation="delivery_pickup_check" data-validation-event="click"  data-validation-error-msg-container="#delivery-pickup-error-dialog" {{ (isset($listing) &&  $listing->delivery == 1 ? 'checked' : '') }}>
              <label for="delivery">
                <i class="fa fa-truck" aria-hidden="true"></i> {{ trans('listings.general.delivery') }}
              </label>
            </div>
            {{-- Delivery costs --}}
            <div class="{{ (isset($listing) &&  $listing->delivery == 1 ? '' : 'hidden') }}" id="delivery_cost">
              {{-- Input for delivery costs --}}
              <div class="input-group">
                <span class="input-group-addon">
                  {{ Currency(Config::get('settings.currency'))->getSymbol() }}
                </span>
                <input type="text" placeholder="{{ trans('listings.form.placeholder.delivery') }}" class="form-control rounded input-lg inline input" name="delivery_price" autocomplete="off" id="delivery_price" value="{{(isset($listing) && $listing->delivery_price != 0 ? old('price',$listing->delivery_price) : null)}}" />
              </div>
              {{-- Delivery Price Info --}}
  ...
  <section class="panel {{ !isset($listing) || isset($listing) && $listing->sell ? '' : 'hidden' }}" id="sell-panel">
    ...
    <div class="panel-body">
      ...
      <div class="input-group input-group-lg">
        ...
        data-validation="number,required" data-validation-ignore=",,." data-validation-error-msg='<div class="alert dark alert-icon alert-danger" role="alert"><i class="icon fa fa-exclamation-triangle" aria-hidden="true"></i> {{ trans('listings.form.validation.price') }}</div>' data-validation-error-msg-container="#price-error-dialog" name="price" id="price" autocomplete="off" value="{{(isset($listing) ? ($listing->price > 0 ? $listing->price : null) : null)}}" placeholder="{{ trans('listings.form.placeholder.sell_price',  ['currency_name' => Currency(Config::get('settings.currency'))->getName()]) }}"/>
      ...
      <div class="payment-system-form {{ ( (isset($listing) && $listing->payment) ? 'show-fees' : '') }} m-t-20">
        <div class="flex-center-space">
          {{-- Payment System head --}}
          <div class="payment-head m-l-10" >
            <i class="fa fa-paypal"></i> {{ trans('payment.paypal_payment') }}
          </div>
        ...
        <div class="payment-fees flex-center-space">
          ...
          <div class="advantages">
            {{-- Secure --}}
            {{ trans('payment.form.secure') }} <i class="fa fa-shield fa-lg text-success"></i><br />
            {{-- Fast --}}
          ...
          <div class="tgame" id="{{$tgame->id}}">
            ...
            <div>
              <span class="form-inline m-r-10">
                ...
                <input type="text" name="trade_list[{{$tgame->id}}][price]" value="{{$add_charge[$tgame->id]['price']}}" class="get_price form-control round  input" placeholder="{{ trans('listings.form.placeholder.additional_charge',  ['currency_name' => Currency(Config::get('settings.currency'))->getName()]) }}" style="{{ $add_charge[$tgame->id]['price_type'] == 'none' ? 'display: none;' : ''  }}">
...
@if(config('settings.payment'))
  ...
  var payment_fees = function(price) {
    var new_price = price;
    var fees = (new_price * {{ config('settings.variable_fee')/100 }}) + {{ config('settings.fixed_fee') }};
  ...
  $("#price").on("change keyup paste click", function(){
    payment_fees($('#price').maskMoney('unmasked')[0]);
  });
...
@if(config('settings.picture_upload') || (isset($listing) && !is_null($listing->picture)))
  ...
  var dropify_upload = $('.dropify').dropify({
      messages: {
          'default': '{{ trans('listings.form.picture_upload.default') }}',
          'replace': '{{ trans('listings.form.picture_upload.replace') }}',
          'remove':  '{{ trans('listings.form.picture_upload.remove') }}',
          'error':   '{{ trans('listings.form.picture_upload.error') }}'
      },
      error: {
          'fileSize': '{{ trans('listings.form.picture_upload.error_filesize') }} (%% value %% max).',
          'minWidth': '{{ trans('listings.form.picture_upload.error_minwidth') }} (%% value %%px min).',
          'maxWidth': '{{ trans('listings.form.picture_upload.error_maxwidth') }} (%% value %%px max).',
          'minHeight': '{{ trans('listings.form.picture_upload.error_minheight') }} (%% value %%px min).',
          'maxHeight': '{{ trans('listings.form.picture_upload.error_maxheight') }} (%% value %%px max).',
          'imageFormat': '{{ trans('listings.form.picture_upload.error_imageformat') }} (%% value %%).',
          'fileExtension': '{{ trans('listings.form.picture_upload.error_fileextension') }} (%% value %%).'
      }
  });

  {{-- Remove image from server and database --}}
  dropify_upload.on('dropify.afterClear', function(event, element){
    $('#picture_remove').val("1");
  });
...
@endif
  ...
  $("#price").maskMoney(maskMoneyOptions);
  ...
  $("#delivery_price").maskMoney(maskMoneyOptions);
  @if(isset($listing) && $listing->delivery_price != 0)
    $("#delivery_price").trigger("mask.maskMoney");
  @endif

  $('.get_price').maskMoney(maskMoneyOptions);
  @if(isset($listing))
    $(".get_price").trigger("mask.maskMoney");
  @endif
  ...
  $('#digital').click(function() {
    if( $.trim($("#digital_distributor").html())=='' ) {
      $.ajax({
        url: '{{ url("api/digitals") }}/' + $(this).data('platform'),
        dataType: "json",
        type: "GET",
        beforeSend: function(){
