@if(count($comments) > 0)
  ...
  <div class="flex-center-space m-b-20">
    ...
    <div>
      {{-- Post comment button --}}
      <a href="#" class="btn btn-dark post-comment"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> {{ trans('comments.post_comment') }}</a>
    </div>
  </div>
  ...
  @foreach($comments as $comment)
    ...
    <div class="comment">
      ...
      <div class="comment-left media-left">
        <a class="avatar avatar-lg" href="{{$comment->user->url}}">
          <img src="{{$comment->user->avatar_square_tiny}}" alt="{{$comment->user->name}} Avatar">
        </a>
      </div>
      ...
      <div class="media-body">
        ...
        <div class="comment-body">
          ...
          <div class="comment-actions">
            {{-- Reply button --}}
            <a href="#" class="reply-comment btn btn-xs btn-dark">
              <i class="fa fa-reply" aria-hidden="true"></i> {{ trans('comments.reply') }}
            </a>
            {{-- Heart icon --}}
            <a class="btn btn-xs @if($comment->dblikes->contains('user_id', \Auth::id())) btn-danger @else btn-dark @endif" id="comment-like" href="{{$comment->id}}">
          ...
          @foreach($childs as $child)
            <div class="reply m-b-20">
                ...
                <div>
                  <a href="{{$child->user->url}}" class="user-link">
                    <span class="avatar avatar-xs">
                      <img src="{{$child->user->avatar_square_tiny}}" alt="{{$child->user->name}}'s Avatar">
                    </span>
                    {{$child->user->name}}
                  </a>
              ...
              <div class="comment-body" style="padding: 0px 10px 10px 10px;">
                ...
                <div class="comment-actions">
                  {{-- Heart icon --}}
                  <a class="btn btn-xs @if($child->dblikes->contains('user_id', \Auth::id())) btn-danger @else btn-dark @endif" id="comment-like" href="{{$child->id}}">
