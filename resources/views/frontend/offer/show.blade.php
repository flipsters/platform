  ...
  @else
    ...
    <div class="offer-status rate-status">
      ...
      <div class="flex-center-space offer-status-wrapper">
        ...
        @if(!$offer->payment && $listing->payment)
...
@if($offer->status >= 1 && (($listing->payment && config('settings.payment') && !isset($trade_game)) || $offer->payment))
    ...
    <div class="payment-fees flex-center-space">
      <div>
        ...
        @else
          @if(Auth::user()->id == $offer->user_id)
            {{-- Pay now button --}}
            <a href="{{url('offer/'.$offer->id.'/pay')}}" class="btn btn-lg btn-success block btn-animate btn-animate-vertical" id="pay-now-button"><span><i class="icon fa fa-paypal"></i> {{ trans('payment.offer.pay_now', ['total' => $offer->price_offer_formatted]) }}</span></a>
            {{-- Protected payment --}}
            <span class="protected-payment"><i class="fa fa-shield fa-lg"></i> {{ trans('payment.offer.protected_payment') }}</span>
          @else
            {{-- Pending --}}
          ...
          <a class="profile-link flex-center" href="{{$offer->user->url}}">
            ...
            <div>
              ...
              @if($offer->user->location)
                <img src="{{ trans('general.url') . 'img/flags/' .   $offer->user->location->country_abbreviation . '.svg' }}" height="14"/> {{$offer->user->location->country_abbreviation}}, {{$offer->user->location->place}} <span class="postal-code">{{$offer->user->location->postal_code}}</span>
              @endif
              </span>
            </div>
          </a>
    ...
    <div class="panel-body">
      <div class="flex-center-space">
        <a class="profile-link flex-center" href="{{$user->url}}">
          {{-- User Avatar --}}
          <span class="avatar m-r-10">
            <img src="{{$user->avatar_square}}" alt="{{$user->name}}'s Avatar">
          </span>
          {{-- User Name & Location --}}
          <div>
            <span class="profile-name small">
            ...
            <span class="profile-location small">
            @if($user->location)
              <img src="{{ trans('general.url') . 'img/flags/' .   $user->location->country_abbreviation . '.svg' }}" height="14"/> {{$user->location->country_abbreviation}}, {{$user->location->place}} <span class="postal-code">{{$user->location->postal_code}}</span>
            @endif
            </span>
          </div>
        </a>
...
@endif
  ...
  <div class="panel offer-chat">
    ...
    <div class="panel-body">
      <div class="chat-box">
        {{-- Load chat via ajax --}}
        <div class="chats" id="ajaxchat" style="height:400px; overflow:scroll;">
        </div>
      </div>
      {{-- Chat input --}}
  ...
  <section class="panel">
    <div class="panel-body">
      @if(!$offer->declined && $offer->status < 2)
        ...
        <a class="btn btn-dark m-b-5 m-t-5" href="{{ url('offer/admin/' . $offer->id . '/close') }}">
          <i class="icon fa fa-tag" aria-hidden="true"></i> Close Offer & Listing
        </a>
        {{-- Close offer & reopen listing button --}}
        <a class="btn btn-dark m-b-5 m-t-5" href="{{ url('offer/admin/' . $offer->id . '/close/reopen') }}">
      ...
      @if($listing->user->id != Auth::user()->id)
        <a class="btn btn-dark m-b-5 m-t-5" href="{{ url('offer/admin/' . $offer->id . '/ban/' . $listing->user->id) }}">
      ...
      @if($offer->user->id != Auth::user()->id)
        <a class="btn btn-dark m-b-5 m-t-5" href="{{ url('offer/admin/' . $offer->id . '/ban/' . $offer->user->id) }}">
      ...
      <span class="staff-tools-seperator"></span>
      {{-- Close report --}}
      <a class="btn btn-dark m-b-5 m-t-5" href="{{ url('offer/admin/report/close/' . $offer->id) }}">
      ...
      <span class="staff-tools-seperator"></span>
        {{-- Refund money button --}}
        @if($offer->payment->status && $offer->payment->transactions()->count() == 0)
          <a class="btn btn-dark m-b-5 m-t-5" href="{{ url('offer/' . $offer->id . '/pay/refund') }}" id="refund-money">
            <i class="icon fa fa-money" aria-hidden="true"></i> Refund money to {{$offer->payment->user->name}}
          </a>
        @endif
        {{-- Release money button --}}
        @if($offer->payment->transactions()->count() == 0 && $offer->payment->status)
          <a class="btn btn-dark m-b-5 m-t-5" href="{{ url('offer/' . $offer->id . '/pay/release') }}" id="release-money">
            <i class="icon fa fa-money" aria-hidden="true"></i> Release money to {{$listing->user->name}}
          </a>
        @endif
      @endif
    </div>
